...
/**
 * 
 * @className	： BreakpointFileStreamResult
 * @description: 文件结果流处理;支持断点续传文件输出流，对StreamResult做了增强，支持断点续传方式（多线程）下载同时也支持普通方式（单线程）下载
 * <b>Example:</b>
 * <pre>
 ...
 * @version 	V1.0
 */
public class BreakpointFileStreamResult extends FileStreamResult {

	private static final long serialVersionUID = -256643510497634924L;
	protected static Logger LOG = LoggerFactory.getLogger(BreakpointFileStreamResult.class);
		...
		try {
			...
			if (endPoint > 0) {
				...
				while (pFrom < endPoint) {
					readLen = randomFile.read(outBuff, 0, readBufSize);
					pFrom += readBufSize;
				...
				LOG.debug("开区间下载开始..."); 
				while ((readLen = randomFile.read(outBuff, 0, bufferSize)) != -1) {
					buffOutput.write(outBuff, 0, readLen);
				}
			}
			buffOutput.flush();
